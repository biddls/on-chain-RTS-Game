...
interface IALCX_map {
    ...
    // other
    function _batchedFromToDAONFT(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory _data) external;    // abstraction function to move a dao NFT
    function _fromToDAONFT(address from, address to, uint256 id, uint256 amount, bytes memory _data) external;
